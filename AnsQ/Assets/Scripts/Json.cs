        ...
        foreach (var item in soruText)
        {
            do
            {
                ...
                if (!result.Any(x => x == zart.question))
                {
                    ...
                    item.transform.parent.parent.GetChild(1).GetChild(0).GetComponent<TMP_Text>().text = zart.option1;
                    item.transform.parent.parent.GetChild(2).GetChild(0).GetComponent<TMP_Text>().text = zart.option2;
                    break;
                }
